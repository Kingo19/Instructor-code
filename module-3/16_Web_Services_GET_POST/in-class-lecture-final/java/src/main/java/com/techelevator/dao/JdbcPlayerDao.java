...
public class JdbcPlayerDao implements PlayerDAO{
    ...
    public List<Player> fetchAllPLayers() {
        ...
        String sql = "SELECT player_id, image_url, firstname, lastname, jerseynumber, salary, team_id FROM players";
        try {
    ...
    public Player addPLayer(Player newPlayer) {
        ...
        String player_table_sql = "INSERT INTO players (firstname, lastname, jerseynumber, salary, team_id) VALUES (?, ?, ?, ?, ?) RETURNING player_id";
        int new_id = jdbcTemplate.queryForObject(player_table_sql, int.class, newPlayer.getFirstName(), newPlayer.getLastName(), newPlayer.getJerseyNumber(), newPlayer.getSalary(), newPlayer.getTeamId());
        newPlayer.setPlayerId(new_id);

        if (newPlayer.getImageURL() == null || newPlayer.getImageURL().equals("")) {
            //set the default image
            newPlayer.setImageURL("https://res.cloudinary.com/drr6cwmiz/image/upload/v1701191329/mystery_exnajf.webp");
        }


        //insert into player_position table
        //TODO - We need to select the position id based on position coming in, and then insert that and the newly provided playerId
        // into the player_position table.
